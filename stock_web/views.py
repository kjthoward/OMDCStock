...
def search(httprequest):
    if httprequest.method=="POST":
        ...
        form = SearchForm(initial = {"inc_open":1})
            ...
            if form.is_valid():
                ...
                if item.reagent.count_no<item.reagent.min_count:
                    if item.sol is not None:
                        ...
                        make="ordered"
                    message+=["Current unopened stock level for {} is {}µl. Minimum quantity is {}µl. Check if more needs to be {}".format(item.reagent.name,item.reagent.count_no,
                                                                                                                          item.reagent.min_count, make)]


                    if EMAIL==True:
                        subject="{} - Unopened Stock Level is below minimum level".format(item.reagent.name)
                ...
                if item.reagent.track_vol==False:
                    if item.reagent.count_no<item.reagent.min_count:
                        if item.sol is not None:
                            ...
                            make="ordered"
                        messages.success(httprequest, "Current unopened stock level for {} is {}. Minimum quantity is {}. Check if more needs to be {}".format(item.reagent.name,
                ...
                if item.reagent.track_vol==True or item.is_op==False:
                    if item.reagent.count_no<item.reagent.min_count:
                        if item.sol is not None:
                            ...
                            make="ordered"
                        messages.success(httprequest, "Current unopened stock level for {0} is {1}{2}. Minimum quantity is {3}{2}. Check if more needs to be {4}".format(item.reagent.name,
                                                                                                                                              item.reagent.count_no,
